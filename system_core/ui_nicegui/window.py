...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Audion Doc to PDF GUI window.")
...
def write_window_log(message: str) -> None:
    try:
        logs = ROOT / "logs"
        logs.mkdir(parents=True, exist_ok=True)
        with (logs / "gui_window.log").open("a", encoding="utf-8") as fh:
...
def main() -> int:
    ...
    if not wait_for_server(args.host, port):
        message = f"GUI server did not start: {url}"
        write_window_log(message)
        print(message)
        stop_server(server)
        return 1
    ...
    try:
        ...
        write_window_log(f"pywebview is not available: {exc}")
    ...
    try:
        webview.create_window("Audion Doc to PDF", url, width=1280, height=800, resizable=False)
        webview.start(
            gui=args.gui,
            debug=False,
            private_mode=False,
        ...
        write_window_log(f"pywebview failed: {exc}")
